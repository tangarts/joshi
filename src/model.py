...
class BlackScholes(Param):
    ...
    def __init__(self, spot0, strike, sigma, r, delta, T, opt_type=None, exer_type=None):
    ...
    def d1(self):
        return (log(self.spot0/self.strike)
    ...
    def vanilla_price(self):
        if self.opt_type == self.OptionType.CALL:
            ...
            return self.strike*exp(-self.r*self.T)*norm.cdf(-self.d2())\
    ...
    def Gamma(self):
        return exp(-self.delta*self.T)*norm.pdf(self.d1())\
    ...
    def Theta(self):
        if self.opt_type == 'call':
            return self.delta*self.spot0*exp(-self.delta*self.T)*norm.cdf(self.d1()) \
                    - (exp(-self.r*self.T)*self.strike*norm.pdf(self.d2())*self.sigma) / (2*sqrt(self.T)) \
    ...
    def perpetual(self):
        if self.opt_type == 'call':
            ...
            return self.strike/(h1 - 1) * (self.spot0/self.strike * (h1 - 1) / h1)**h1, H
            ...
            return self.strike/(1 - h2) * (self.spot0/self.strike * (h2 - 1) / h2)**h2, H

    @staticmethod
    def gap_option(S, strike1, strike2, sigma, r, delta, T):
        """ compute gap option """

        d1 = log(S*exp(-delta*T)/(strike2*exp(-r*T))) + 0.5*sigma**2*T \
                / sigma*sqrt(T)
        d2 = d1 - sigma*sqrt(T)
    ...
    def exchange_option(S, strike, sig_s, sig_k, delta_s, delta_k, rho, T):
        ...
        d1 = log(S*exp(-delta_s*T)/(strike*exp(-delta_k*T))) + 0.5*sigma**2*T \
                / sigma*sqrt(T)

        d2 = d1 - sigma*sqrt(T)

        return S*exp(-delta_s*T)*norm.cdf(d1) - strike*exp(-delta_k*T)*norm.cdf(d2)
...
class Numerical(Param):
    ...
    def __init__(self, spot0, strike, sigma, r, d, T, opt_type=None, exer_type=None, M=None, N=None):
        """
        args:
            Params
        """
    ...
    def error(self, steps):
        exact = BlackScholes(self.spot0, self.strike, self.sigma, self.r,\
...
class Binomial(Param):
    ...
    def __init__(self, spot0, strike, sigma, r, delta, T, opt_type=None, exer_type=None, N=None):
        super().__init__(spot0, strike, sigma, r, delta, T, opt_type, exer_type)
    ...
    def european(self):
        ...
        if self.opt_type == 'call':
            option[:, self.N] = np.maximum(np.zeros(self.N + 1), (stock[:, self.N] - self.strike))
            ...
            option[:, self.N] = np.maximum(np.zeros(self.N + 1), (self.strike - stock[:, self.N])) # put payoff
            for i in range(self.N - 1, -1, -1):
                for j in range(0, i + 1):
                    option[j, i] = exp(-self.r*self.h)\
                            * (self.q*option[j, i + 1]\
    ...
    def american(self):
        ...
        if self.opt_type == 'call':
            ...
            option[:, self.N] = np.maximum(np.zeros(self.N + 1), (self.strike - stock[:, self.N]))
            for i in range(self.N -1, -1, -1):
                for j in range(0, i + 1):
                    option[j, i] = exp(-self.r*self.h)\
                            * (self.q*max(option[j, i + 1], payoff[j, i + 1])\
